...
*/
      //  std::cout<<"c:"<<c<<" len="<<len<<std::endl;
        //if ( !host.Recv(c, len) ) return;//将报文读出，并从接收缓冲中删除，绝对不可能长度不够，即使连接>已经断开，也可以读到数据
        if ( !host.Recv(c, 2) ) return;//将报文读出，并从接收缓冲中删除，绝对不可能长度不够，即使连接已经>断开，也可以读到数据
       // std::cout<<"c:"<<c<<" len="<<len<<std::endl;
        //host.Send( c, len );//收到消息原样回复
        host.Send( c, 2 );//收到消息原样回复
}
